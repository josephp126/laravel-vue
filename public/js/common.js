...
//----HEADER---//
    jQuery(window).scroll(function () {
        console.log('parseInt(jQuery(document).scrollTop()', parseInt(jQuery(document).scrollTop()));
        if (parseInt(jQuery(document).scrollTop()) > 35) {
            jQuery('.wowmenu').addClass(' navbar-fixed-top');
        } else {
            jQuery('.wowmenu').removeClass(' navbar-fixed-top');
        }
    });
...
jQuery(document).ready(function ($) {
    ...
    jQuery(document.body).on('appear', '.entrance', function () {
        jQuery(this).
            each(function () { jQuery(this).addClass('anim-entrance'); });
    });
    ...
    jQuery(document.body).on('appear', '.fadeInRightNow', function () {
        jQuery(this).
            each(function () { jQuery(this).addClass('fadeInRight'); });
    });

    jQuery(document.body).on('appear', '.fadeInUpBigNow', function () {
        jQuery(this).
            each(function () { jQuery(this).addClass('fadeInUpBig'); });
    });
    jQuery(document.body).on('appear', '.fadeInDownBigNow', function () {
        jQuery(this).
            each(function () { jQuery(this).addClass('fadeInDownBig'); });
    });
    jQuery(document.body).on('appear', '.fadeInLeftBigNow', function () {
        jQuery(this).
            each(function () { jQuery(this).addClass('fadeInLeftBig'); });
    });
    jQuery(document.body).on('appear', '.fadeInRightBigNow', function () {
        jQuery(this).
            each(function () { jQuery(this).addClass('fadeInRightBig'); });
    });
